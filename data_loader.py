...
class DataLoader:
    ...
    def load_transform_file(self, file_path):
        ...
        with open(save_loc, 'r') as file:
            session.run("MATCH (pick) DETACH DELETE pick")
            session.run("MATCH (drop) DETACH DELETE drop")
            session.run("MATCH (rel) DETACH DELETE rel")
            reader = csv.DictReader(file)
            for row in reader:
                # create nodes and relationship
                session.run('MERGE (pick:Location {name: $pulid}) RETURN pick', pulid=int(row['PULocationID']))
                session.run('MERGE (drop:Location {name: $dolid}) RETURN drop', dolid=int(row['DOLocationID']))
                relationship = '''MATCH (pick:Location {name: $pulid})
                MATCH (drop:Location {name: $dolid})
                MERGE (pick)-[rel:TRIP {distance: $distance, fare: $fare, pickup_dt: datetime($ptime), dropoff_dt: datetime($dtime)}]->(drop)'''
                session.run(relationship, pulid=int(row['PULocationID']), dolid=int(row['DOLocationID']), distance=float(row['trip_distance']), fare=float(row['fare_amount']), ptime=row['tpep_pickup_datetime'].replace(" ","T"), dtime=row['tpep_dropoff_datetime'].replace(" ","T"))
